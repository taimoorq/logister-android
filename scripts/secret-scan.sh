...
sensitive_untracked="$(git ls-files --others --exclude-standard | grep -E '(^|/)(\.env(\..*)?|local\.properties|gradle-local\.properties|credentials\.properties|secring\..*|signing-key\..*|private-key\..*|.*\.(asc|jks|keystore|p12|pem|key|gpg|pgp|kbx))$' || true)"
